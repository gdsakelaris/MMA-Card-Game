...
- **Knockdown** — only the two heaviest shots, **Head Kick** and **Heavy Hook**, can knock down — every other strike (Jab, Teep, Leg Kick, Body Shot, Power Cross, Cutting Elbow, Uppercut, Spinning Back Fist) never does, no matter how hard it lands. A knockdown hit whose **delivered power** (base **+ Striking**, plus any training buff, minus leg damage **and minus any Block reduction**) reaches **8 or more** **drops the opponent**: they go to the **bottom** and **you take top control**. The **combo bonus does NOT count** toward a knockdown — combos add damage, but a knockdown is about the weight of the single shot, so padding it with a combo won't drop anyone (heavier base + a better striker = more reliable knockdown). Because it's about **delivered power**: a full **negate** (Parry / Slip) stops it cold, and a **Block** stops it if its −3 brings the shot below 8 (it usually saves you from a Heavy Hook, but rarely from a Head Kick). Now follow them down — Ground & Pound, or lock in a Rear Naked Choke.
...
| Uppercut | punch | 5 | 3 | 2 | heavy short punch; legal in the clinch |
...
*Last updated: Version 4.9 — Clinch striking, Check Kick punish, knockdown-by-delivered-power, flying-sub gamble. **More strikes work in the clinch:** Body Shot and Heavy Hook join Uppercut and Cutting Elbow as legal short inside strikes (the Clinch card text now lists them; long-range jabs/kicks/Head Kick still need open space). **Check Kick counter raised 2 → 4** — eating a checked kick hurts more than a slipped punch (it now out-counters Slip, the trade-off being it only answers kicks). **Knockdowns trimmed and reworked.** Only the two heaviest shots — **Head Kick and Heavy Hook** — can now knock down (Uppercut and Power Cross no longer do, so they keep just their own identities: a clinch-legal heavy punch, and "a Block can't soften it"). Knockdown is now about **DELIVERED power**: the shot's own power (base + Striking − leg damage − any Block reduction, combo excluded) reaching 8+ — so a full negate (Parry/Slip) stops it cold and a Block stops it if its −3 drops the shot below 8 (this also fixed the old inconsistency where a knockdown was gated on "was a card played" rather than on whether the hit actually landed). **Defended flying submission now sweeps the attacker under:** a flying sub from the clinch that's fully defended hands the *defender* top control (the attacker lands on the bottom) instead of rewarding the attacker — throwing a flying sub is now a real positional gamble. AI no longer wastes the Stand Up card from top control (it stands for free). One ruleset for digital and physical.*
