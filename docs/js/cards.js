        ...
        const cardDatabase = {
            ...
            techniques: [
                ...
                { id: 'tech_legkick',  name: 'Leg Kick',           type: 'technique', subtype: 'strike', strikeType: 'kick',  energy: 1, damage: 3, copies: 3, onHitStatus: { type: 'legDamage', amount: 1 }, effect: 'Strike: 3 + Striking. Chops the legs: stacking -1 to ALL their strikes (clean hit).' },
                { id: 'tech_bodyshot', name: 'Body Shot',          type: 'technique', subtype: 'strike', strikeType: 'punch', energy: 2, damage: 3, copies: 3, energyDrain: 2, effect: 'Strike: 3 + Striking. To the body: saps 2 energy (cardio) on a clean hit.' },
                { id: 'tech_cross',    name: 'Power Cross',        type: 'technique', subtype: 'strike', strikeType: 'punch', energy: 2, damage: 4, copies: 2, ignoresReduce: true, canKnockdown: true, effect: 'Strike: 4 + Striking. Straight power — a Block can\'t soften it (only a full Parry/Slip stops it). A clean 8+ knocks down.' },
                { id: 'tech_elbow',    name: 'Cutting Elbow',      type: 'technique', subtype: 'strike', strikeType: 'elbow', energy: 2, damage: 4, copies: 2, clinchLegal: true, groundStrike: true, onHitStatus: { type: 'bleed', amount: 2, turns: 2 }, effect: 'Strike: 4 + Striking. Bleed (2 dmg for 2 turns) on a clean hit. Legal in the clinch and from top control.' },
                { id: 'tech_uppercut', name: 'Uppercut',           type: 'technique', subtype: 'strike', strikeType: 'punch', energy: 3, damage: 5, copies: 2, clinchLegal: true, canKnockdown: true, effect: 'Strike: 5 + Striking. A short inside punch — also legal in the clinch. A clean 8+ knocks down.' },
                { id: 'tech_hook',     name: 'Heavy Hook',         type: 'technique', subtype: 'strike', strikeType: 'punch', energy: 3, damage: 6, copies: 2, canKnockdown: true, effect: 'Strike: 6 + Striking. Loads up: a clean 8+ (combo it!) scores a KNOCKDOWN.' },
                ...
                { id: 'tech_clinch',   name: 'Clinch',             type: 'technique', subtype: 'clinch', energy: 1, copies: 3, effect: 'Tie up in the clinch. From the clinch you can hit a flying submission, shoot a takedown, or stand up.' },
                ...
                // -- vs strikes --
                { id: 'tech_block', name: 'Block',         type: 'technique', subtype: 'defense', reactsTo: ['strike'], mitigation: 'reduce', reduction: 3, energy: 1, copies: 4, effect: 'Reaction to a strike: cover up, reduce it by 3.' },
                { id: 'tech_parry', name: 'Parry',         type: 'technique', subtype: 'defense', reactsTo: ['strike'], mitigation: 'negate', energy: 2, copies: 2, effect: 'Reaction to a strike: deflect it completely (no damage).' },
                { id: 'tech_slip',  name: 'Slip Counter',  type: 'technique', subtype: 'defense', reactsTo: ['strike'], mitigation: 'negate', counter: 3, energy: 2, copies: 2, effect: 'Reaction to a strike: slip it (no damage) and counter for 3.' },
                { id: 'tech_checkkick', name: 'Check Kick', type: 'technique', subtype: 'defense', reactsTo: ['strike'], reactsToStrikeType: 'kick', mitigation: 'negate', counter: 2, energy: 1, copies: 2, effect: 'Reaction to a KICK only: check it (no damage) and counter for 2.' },
                // -- vs takedowns --
                { id: 'tech_stuff',  name: 'Stuff',           type: 'technique', subtype: 'defense', reactsTo: ['grappling'], mitigation: 'negate', energy: 1, copies: 3, effect: 'Reaction to a takedown: stuff it, stay standing.' },
                { id: 'tech_sprawl', name: 'Sprawl',          type: 'technique', subtype: 'defense', reactsTo: ['grappling'], mitigation: 'negate', counter: 2, energy: 2, copies: 2, effect: 'Reaction to a takedown: sprawl, stay standing and counter for 2.' },
                { id: 'tech_counterTd', name: 'Counter Takedown', type: 'technique', subtype: 'defense', reactsTo: ['grappling'], mitigation: 'reverse', energy: 3, copies: 1, effect: 'Reaction to a takedown: reverse it — YOU take them down to top control.' },
                // -- vs submissions --
                { id: 'tech_subdef', name: 'Submission Defense', type: 'technique', subtype: 'defense', reactsTo: ['submission'], mitigation: 'negate', energy: 2, copies: 2, effect: 'Reaction to a submission: defend the lock completely (no damage).' }
            ],
            corners: [
